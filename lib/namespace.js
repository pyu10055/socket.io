...
SocketNamespace.prototype.handlePacket = function (sessid, packet) {
  ...
  switch (packet.type) {
    ...
    case 'event':
	  ...
	  //when data packets are received on the other nodes, new socket is created,
	  //which socket listener are not aware of that. Call connect to emit proper
      //messages such as 'connection'.
      if (!socket.connected) {        
        connect();
      }  
